 ...
 * @input  Hm         The message hash (in field)// changed Hm to step_in for nova interface
 * @output step_out   The message hash that is the public input Hm as required for nova interface
 */
template AggregateVerify(b, n, k){
    signal input pubkeys[b][2][k];
    signal input pubkeybits[b];
    signal input signature[2][2][k];
    signal input step_in[2][2][k];
    signal output step_out[2][2][k];
    ...
    for (var reg_idx = 0; reg_idx < k; reg_idx++) {
        for (var x_or_y = 0; x_or_y < 2; x_or_y++) {
            ...
            verifySignature.hash[0][x_or_y][reg_idx] <== step_in[0][x_or_y][reg_idx];
            verifySignature.hash[1][x_or_y][reg_idx] <== step_in[1][x_or_y][reg_idx];
        }
    }
}
